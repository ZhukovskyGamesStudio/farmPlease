...
namespace UnityEditor.Tilemaps {
    ...
    public class LineBrush : GridBrush {
        ...
        public override void Paint(GridLayout grid, GameObject brushTarget, Vector3Int position) {
            if (lineStartActive) {
                Vector2Int startPos = new(lineStart.x, lineStart.y);
                Vector2Int endPos = new(position.x, position.y);
                if (startPos == endPos)
                    base.Paint(grid, brushTarget, position);
                else
                    foreach (Vector2Int point in GetPointsOnLine(startPos, endPos, fillGaps)) {
                        Vector3Int paintPos = new(point.x, point.y, position.z);
        ...
        public static IEnumerable<Vector2Int> GetPointsOnLine(Vector2Int startPos, Vector2Int endPos, bool fillGaps) {
            IEnumerable<Vector2Int> points = GetPointsOnLine(startPos, endPos);
            if (fillGaps) {
                int rise = endPos.y - startPos.y;
                int run = endPos.x - startPos.x;

                if (rise != 0 || run != 0) {
                    Vector2Int extraStart = startPos;
                    Vector2Int extraEnd = endPos;
                    ...
                    IEnumerable<Vector2Int> extraPoints = GetPointsOnLine(extraStart, extraEnd);
    ...
    public class LineBrushEditor : GridBrushEditor {
        ...
        public override void OnPaintSceneGUI(GridLayout grid, GameObject brushTarget, BoundsInt position,
            ...
            if (lineBrush.lineStartActive) {
                ...
                Vector2Int startPos = new(lineBrush.lineStart.x, lineBrush.lineStart.y);
                Vector2Int endPos = new(position.x, position.y);
                if (startPos == endPos)
                    PaintPreview(grid, brushTarget, position.min);
                else
                    foreach (Vector2Int point in LineBrush.GetPointsOnLine(startPos, endPos, lineBrush.fillGaps)) {
                        Vector3Int paintPos = new(point.x, point.y, position.z);
                        PaintPreview(grid, brushTarget, paintPos);
                    }

                if (Event.current.type == EventType.Repaint) {
                    Vector3Int min = lineBrush.lineStart;
                    Vector3Int max = lineBrush.lineStart + position.size;

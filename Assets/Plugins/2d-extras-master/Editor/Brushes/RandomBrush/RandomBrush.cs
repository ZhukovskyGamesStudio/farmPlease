    ...
    public class RandomBrush : GridBrush {
        ...
        public override void Paint(GridLayout grid, GameObject brushTarget, Vector3Int position) {
            if (randomTileSets != null && randomTileSets.Length > 0) {
                ...
                foreach (Vector3Int startLocation in new SizeEnumerator(min, min + size, randomTileSetSize)) {
                    RandomTileSet randomTileSet = randomTileSets[(int) (randomTileSets.Length * Random.value)];
                    BoundsInt randomBounds = new BoundsInt(startLocation, randomTileSetSize);
        ...
        public override void Pick(GridLayout gridLayout, GameObject brushTarget, BoundsInt bounds,
            ...
            foreach (Vector3Int startLocation in new SizeEnumerator(bounds.min, bounds.max, randomTileSetSize)) {
                ...
                foreach (Vector3Int pos in randomBounds.allPositionsWithin) {
                    TileBase tile = pos.x < bounds.max.x && pos.y < bounds.max.y && pos.z < bounds.max.z
                        ? tilemap.GetTile(pos)
                        : null;
    ...
    public class RandomBrushEditor : GridBrushEditor {
        ...
        public override void PaintPreview(GridLayout grid, GameObject brushTarget, Vector3Int position) {
            if (randomBrush.randomTileSets != null && randomBrush.randomTileSets.Length > 0) {
                ...
                foreach (Vector3Int startLocation in new RandomBrush.SizeEnumerator(min, min + randomBrush.size,
                             randomBrush.randomTileSetSize)) {
                    RandomBrush.RandomTileSet randomTileSet =
                        randomBrush.randomTileSets[(int) (randomBrush.randomTileSets.Length * Random.value)];
                    BoundsInt randomBounds = new BoundsInt(startLocation, randomBrush.randomTileSetSize);
                    int j = 0;
                    foreach (Vector3Int pos in randomBounds.allPositionsWithin)

...
public class LocalizationPrefs : MonoBehaviour {
    [SerializeField]
    [Tooltip("PlayerPrefs key to keep language preference.")]
    ...
    private void LoadLanguage() {
        ...
        SystemLanguage savedLanguage = GetSavedLanguage();
        if (savedLanguage != SystemLanguage.Unknown) Localization.Instance.CurrentLanguage = savedLanguage;
    }
    ...
    private SystemLanguage GetSavedLanguage() {
        if (PlayerPrefs.HasKey(m_PrefKey)) {
            int languageValue = PlayerPrefs.GetInt(m_PrefKey, -1);
            if (languageValue >= 0) return (SystemLanguage) languageValue;
        }

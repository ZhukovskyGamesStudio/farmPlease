    ...
    public class LocalizedAssetEditor : UnityEditor.Editor {
            ...
            if (m_LocaleItems != null) {
                ...
                m_LocaleItemList.drawElementCallback = (rect, index, isActive, isFocused) => {
                    ...
                    var languageProperty =
                        element.FindPropertyRelative(LocalizationEditorHelper.LocaleLanguageSerializedProperty);
                    EditorGUI.PropertyField(languageRect, languageProperty, GUIContent.none);

                    // Value field.
                    var valueRect = new Rect(languageRect.x + languageRect.width + 4, languageRect.y,
                        rect.width - languageRect.width - 4, rect.height - 4);
                    var valueProperty =
                        element.FindPropertyRelative(LocalizationEditorHelper.LocaleValueSerializedProperty);
                    if (assetValueType == typeof(string)) {
                        valueProperty.stringValue =
                            EditorGUI.TextArea(valueRect, valueProperty.stringValue, m_TextAreaStyle);
                    } else {
                        EditorGUI.PropertyField(valueRect, valueProperty, GUIContent.none);
                    }
                };
                m_LocaleItemList.onCanRemoveCallback = (list) => { return list.count > 1; };
                m_LocaleItemList.elementHeightCallback = (index) => {
                    ...
                    if (assetValueType == typeof(string)) {
                        ...
                        elementHeight =
                            m_TextAreaStyle.CalcHeight(new GUIContent(valueProperty.stringValue), valueWidth);
                    }
                ...
                m_LocaleItemList.onAddDropdownCallback = (Rect buttonRect, ReorderableList list) => {
                    ...
                    menu.AddItem(
                        new GUIContent("Add languages from settings", "Adds by searching used languages in assets."),
                        false, () => {
        ...
        public override void OnInspectorGUI() {
            if (m_LocaleItemList != null) {
                m_CurrentLayoutRect =
                    GUILayoutUtility.GetRect(0, m_LocaleItemList.GetHeight(), GUILayout.ExpandWidth(true));

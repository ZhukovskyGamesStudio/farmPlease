...
public class ClickableObject : MonoBehaviour, IPointerClickHandler, IPointerDownHandler, IPointerUpHandler
{
    ...
    {
		if (eventData.button == PointerEventData.InputButton.Right && (eventData.position - rightpos).magnitude < MaxDistanceDeltaToRegisterClick)
			RightclickEvent.Invoke();
		if (eventData.button == PointerEventData.InputButton.Left && (eventData.position - leftpos).magnitude < MaxDistanceDeltaToRegisterClick)
			LeftclickEvent.Invoke();
	}

...
public class TilesTable : MonoBehaviour
{
	...
	public bool RecreateDictionary = false;
...
{
    None,
    Sand, Soil, Weed, WateredSoil, Rocks,
    TomatoSeed, WateredTomatoSeed, Tomato_1, Tomato_2, Tomato_3,
    EggplantSeed, WateredEggplantSeed, Eggplant_1, Eggplant_2, Eggplant_3,
    CornSeed, WateredCornSeed, Corn_1,
    DandellionSeed, WateredDandelliomSeed, Dandellion_1,
    StrawberrySeed, WateredStrawberrySeed, Strawberry_1,
    CactusSeed, Cactus_1,
    FernSeed, WateredFernSeed, Fern_1, WateredFern_1, FernDead,
    Beautyflowerseed_1, Beautyflowerseed_2, Beautyflowerseed_3, Beautyflowerseed_4, Beautyflowerseed_5, Beautyflowerseed_6, Beautyflowerseed_7,
    WateredBeautyflowerseed_1, WateredBeautyflowerseed_2, WateredBeautyflowerseed_3, WateredBeautyflowerseed_4, WateredBeautyflowerseed_5, WateredBeautyflowerseed_6, WateredBeautyflowerseed_7,
    Beautyflower_1, BeautyflowerDead, BeautyflowerSibling,
    FlycatherSeed_1, FlycatherSeed_2, FlycatherSeed_3, Flycather_1,
    OnionSeed, OnionWatered, Onion_1,
    Biogen_empty, Biogen_full, Biogen_T1, Biogen_T2, Biogen_T3, Biogen_Construction,
    Freshener_empty, Freshener_1, Freshener_2, Freshener_3, Freshener_4, Freshener_5, Freshener_6, Freshener_full, Freshener_Construction, Freshener_T1, Freshener_T2, Freshener_T3,
    Sprinkler_empty, Sprinkler_1, Sprinkler_2, Sprinkler_3, Sprinkler_4, Sprinkler_5, Sprinkler_full, Sprinkler_target, Sprinkler_Construction, Sprinkler_T1, Sprinkler_T2, Sprinkler_T3,
    SeedDoubler_empty, SeedDoubler_full, SeedDoubler_Construction, SeedDoubler_T1, SeedDoubler_T2, SeedDoubler_T3,
    Tractor_1, Tractor_2, Tractor_Construction, Tractor_T1, Tractor_T2, Tractor_T3,
    Pumpkinseed_1, Pumpkinseed_2, Pumpkinseed_3, WateredPumpkinseed_1, WateredPumpkinseed_2, WateredPumpkinseed_3, GrownPumpkinseed_1, GrownPumpkinseed_2, GrownPumpkinseed_3, Pumpkin_1, Pumpkin_2, Pumpkin_3,
    RadishSeed, WateredRadishSeed, Radish_1,
    PeanutSeed, WateredPeanutSeed, Peanut_1, PeanutDead
}
...
public class SmartTile : MonoBehaviour
{
	...
	public bool CanBeHoed()
	{
		...
		if (TilesTable.TileByType(type).CanBeHoed)
			return true;
		else {
			if (type == TileType.Strawberry_1)
			{
				...
				List<SmartTile> neighborsL = new List<SmartTile>();
	...
	public bool CanBeSeeded()
	{
		...
		else 		if (TilesTable.TileByType(type).CanBeSeeded)
			return true;
		else { }
		return false;
	}
	...
	public bool CanBeWatered()
	{
		...
		if (TilesTable.TileByType(type).TIndex > 0)
			return tilemap.GetBuildingByPart(position).CanBeWatered();
		else if (TilesTable.TileByType(type).CanBeWatered)
			return true;
		else {
	...
	public bool CanBeCollected()
	{
		...
		if (TilesTable.TileByType(type).CanBeCollected)
			return true;
		else { }
		return false;
	...
	public bool CanbeNewDayed()
	{
		if (!isActive)
			return false;
		if (TilesTable.TileByType(type).CanBeNewDayed)
			return true;
		else {
	...
	public bool CanBeErosioned()
	{
		if (!isActive)
			return false;
		if (TilesTable.TileByType(type).CanBeErosioned)
			return true;
		else { }
		return false;
	}
	...
	public bool CanBeClicked()
	{
		...
		else  if (TilesTable.TileByType(type).CanBeClicked)
			return true;
		else { }
		return false;
	}
	...
	public bool IsBuilding()
	{
		...
		if (TilesTable.TileByType(type).IsBuilding)
			return true;
		else { }
		return false;
	}
	...
	public IEnumerator OnHoed(float animtime)
	{
		...
		if (neighborTiles[3].type == TileType.Radish_1 && tilemap.GetHexNeighbors(neighborTiles[3].position)[5].CanBeHoed())
			yield return tilemap.GetHexNeighbors(neighborTiles[3].position)[5].OnHoed( animtime);

		if (neighborTiles[4].type == TileType.Radish_1 && tilemap.GetHexNeighbors(neighborTiles[4].position)[4].CanBeHoed())
			yield return tilemap.GetHexNeighbors(neighborTiles[4].position)[4].OnHoed( animtime);

		if (neighborTiles[5].type == TileType.Radish_1 && tilemap.GetHexNeighbors(neighborTiles[5].position)[3].CanBeHoed())
    ...
    public IEnumerator OnSeeded(CropsType seedtype, float animtime)
    {
        ...
        if (data.TIndex == 1)
            yield return StartCoroutine(tilemap.GetTile(position + new Vector3Int(1, -1, 0)).OnSeeded(seedtype, animtime));
        else if (data.TIndex == 2)
            yield return StartCoroutine(tilemap.GetTile(position + new Vector3Int(0, -1, 0)).OnSeeded(seedtype, animtime));
        else if (data.TIndex == 3)
            yield return StartCoroutine(tilemap.GetTile(position + new Vector3Int(-1, 0, 0)).OnSeeded(seedtype, animtime));
        else
            switch (type)
            {
                ...
                case TileType.Soil:
                    switch (seedtype)
                    {
						...
						case CropsType.Pumpkin:
							...
							SmartTile[] neighbors = tilemap.GetHexNeighbors(position);
							List<SmartTile> pumpList = new List<SmartTile>();
                            for (int i = 0; i < neighbors.Length; i++)
                            {
        ...
        {
			if (neighborTiles[3].type == TileType.Radish_1 && tilemap.GetHexNeighbors(neighborTiles[3].position)[5].CanBeSeeded())
            {
			...
			if (neighborTiles[4].type == TileType.Radish_1 && tilemap.GetHexNeighbors(neighborTiles[4].position)[4].CanBeSeeded())
            {
				yield return tilemap.GetHexNeighbors(neighborTiles[4].position)[4].OnSeeded(seedtype, animtime);
				InventoryManager.instance.LoseSeed(seedtype);
			}	  				

			if (neighborTiles[5].type == TileType.Radish_1 && tilemap.GetHexNeighbors(neighborTiles[5].position)[3].CanBeSeeded())
            {
    ...
    public IEnumerator OnWatered(float animtime)
	{
		...
		if (neighborTiles[3].type == TileType.Radish_1 && tilemap.GetHexNeighbors(neighborTiles[3].position)[5].CanBeWatered())
			yield return tilemap.GetHexNeighbors(neighborTiles[3].position)[5].OnWatered( animtime);

		if (neighborTiles[4].type == TileType.Radish_1 && tilemap.GetHexNeighbors(neighborTiles[4].position)[4].CanBeWatered())
			yield return tilemap.GetHexNeighbors(neighborTiles[4].position)[4].OnWatered( animtime);

		if (neighborTiles[5].type == TileType.Radish_1 && tilemap.GetHexNeighbors(neighborTiles[5].position)[3].CanBeWatered())
	...
	public IEnumerator OnCollected(bool isPlayerHaveGreenScythe,float animtime)
	{
		...
		{
			if (neighborTiles[i].type == TileType.Fern_1 && type != TileType.Onion_1 || neighborTiles[i].type == TileType.WateredFern_1 && type != TileType.Onion_1)
				multiplier = 2;
		}
        ...
        {
			InventoryManager.instance.CollectCrop(TilesTable.TileByType(type).CropCollected, TilesTable.TileByType(type).collectAmount * multiplier);
			SwitchType(TileType.Soil, AnimationType.Hoe);
		}
		else
            switch (type)
		...
		if (neighborTiles[3].type == TileType.Radish_1 && tilemap.GetHexNeighbors(neighborTiles[3].position)[5].CanBeCollected())        
			yield return tilemap.GetHexNeighbors(neighborTiles[3].position)[5].OnCollected(isPlayerHaveGreenScythe,animtime);

		if (neighborTiles[4].type == TileType.Radish_1 && tilemap.GetHexNeighbors(neighborTiles[4].position)[4].CanBeCollected())
			yield return tilemap.GetHexNeighbors(neighborTiles[4].position)[4].OnCollected(isPlayerHaveGreenScythe, animtime);

		if (neighborTiles[5].type == TileType.Radish_1 && tilemap.GetHexNeighbors(neighborTiles[5].position)[3].CanBeCollected())
			yield return tilemap.GetHexNeighbors(neighborTiles[5].position)[3].OnCollected(isPlayerHaveGreenScythe, animtime);
	...
	public IEnumerator OnNeyDayed(float animtime)
	{
		...
		if (TilesTable.TileByType(type).NewDaySwitch != TileType.None)
			SwitchType(TilesTable.TileByType(type).NewDaySwitch, AnimationType.None);
        else
            switch (type)
            {
                ...
                case TileType.WateredCornSeed:
                    ...
                    List<SmartTile> soilTiles = new List<SmartTile>();
                    ...
                    if (soilTiles.Count > 0)
                    {
                        int rnd = UnityEngine.Random.Range(0, soilTiles.Count);
                ...
                case TileType.FlycatherSeed_1:
                    ...
                    for (int i = 0; i < 1; i++)
                    {
                        ...
                        {
                            SmartTile tile = toSeedList[UnityEngine.Random.Range(0, toSeedList.Count)];
				...
				case TileType.Tractor_1:  					
                    ...
                    {
						SwitchType(TileType.Tractor_2, AnimationType.None);
						yield return sandTiles[Random.Range(0, sandTiles.Length)].OnHoed(animtime * 10);
						SwitchType(TileType.Tractor_1, AnimationType.None);
					}
					break;
				...
				case TileType.WateredPeanutSeed:
					...
					if (soilTiles.Count > 0)
					{
						...
						for (int i = 0; i < surroundingtiles.Length; i++)
						{
							if (!TilesTable.TileByType(surroundingtiles[i].type).IsBuilding && TilesTable.TileByType(surroundingtiles[i].type).CropType != CropsType.Peanut && TilesTable.TileByType(surroundingtiles[i].type).CropType != CropsType.None)
                            {
	...
	public IEnumerator OnErosioned(float animtime)
	{
		...
		switch (type)
		{
			case TileType.Freshener_full:
				SwitchType(TileType.Freshener_empty); break;
			case TileType.Soil:
				SwitchType(TileType.Sand); break;
			case TileType.WateredSoil:
				SwitchType(TileType.Weed); break;
			case TileType.CactusSeed:
				SwitchType(TileType.Cactus_1); break;
		}
	...
	public IEnumerator OnInsected(float animtime)
	{
		...
		{
			BecomeInactive();
			SwitchType(TileType.Flycather_1, AnimationType.None);
			yield break;
		}
		else if (CanBeCollected())
		{
			BecomeInactive();
			SwitchType(TileType.Soil, AnimationType.None);
		}
		yield return new WaitForSeconds(animtime);
		BecomeActive();
        ...
        if (data.TIndex == 1)
            ...
            switch (type)
            {
                case TileType.Biogen_empty:
                    ...
                    List<SmartTile> weedArea = new List<SmartTile>
                {
                    ...
                    while (weedArea.Count > 0)
                    {
                        ...
                        {
                            SwitchType(TileType.Biogen_full, AnimationType.None);
                            tile.SwitchType(TileType.Soil, AnimationType.None);

                            yield return new WaitForSeconds(1.5f);
							PlayerController.instance.RestoreEnergy(1);
                            SwitchType(TileType.Biogen_empty, AnimationType.None);
                            break;
                        }
